...
public class MemberController {
                       ...
                       @RequestParam(value = "page",defaultValue = "1") int page)
    {
        ...
        // 전체 조회 리스트
        List<MemberDto> lists = memberMapper.selectAllMember(params);
    ...
    public String updateProc(@ModelAttribute("mDto") @Valid  MemberDto mDto, BindingResult br ,Model model,
        ...
        if (br.hasErrors()) {
            ...
            resultPage = "redirect:/mlist.mb?page="+page+"&whatColumn="+whatColumn+"&keyword="+encodeKeyword;
        }
        return resultPage;
    }
    ...
    public String loginProc(MemberDto mDto, HttpServletRequest request, HttpServletResponse response, HttpSession session) throws IOException { // id , password
        ...
        // 브라우저 응답을 HTML + UTF-8 로 설정 : 한글 깨짐 방지 (반드시 getWriter() 전에 호출).
        response.setContentType("text/html;charset=UTF-8");

        // 브라우저에 직접 출력하기 위한 Writer 준비 (브라우저와 연결다리)
        PrintWriter pw;
